...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/opensy-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
...
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/OpenSY"
else
  echo > "${HOME}/.opensy"
fi
...
OPENSY_CONFIG_ALL="-DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  OPENSY_CONFIG_ALL="${OPENSY_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  OPENSY_CONFIG_ALL="${OPENSY_CONFIG_ALL} -DWERROR=ON"
fi
...
OPENSY_CONFIG_ALL="$OPENSY_CONFIG_ALL -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR -Werror=dev"

if [[ "${RUN_TIDY}" == "true" ]]; then
  OPENSY_CONFIG_ALL="$OPENSY_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

eval "CMAKE_ARGS=($OPENSY_CONFIG_ALL $OPENSY_CONFIG)"
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/opensy-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target opensy-tidy-tests "$MAKEJOBS"
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libopensy-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
  ...
  run_iwyu() {
    ...
    python3 "/include-what-you-use/iwyu_tool.py" \
             ...
             -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/opensy.core.imp" \
