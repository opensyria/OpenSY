...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the OpenSyria network. This means one can have an always-on opensyriad instance running, and another local opensyriad/opensyria-qt instance which connects to this node and receives blocks from it.
...
This script attempts to download and verify the signature file SHA256SUMS.asc from opensyria.org.
...
This script converts a compact-serialized UTXO set (as generated by OpenSyria Core with `dumptxoutset`)
