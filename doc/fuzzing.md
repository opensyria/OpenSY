# Fuzzing OpenSY using libFuzzer
...
## Overview of OpenSY fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/opensy_report.pdf) includes an external overview of the status of OpenSY fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.
...
It is possible to specify `opensyd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`opensy-core/qa-assets`](https://github.com/opensy-core/qa-assets) repo.

To fuzz `process_message` using the [`opensy-core/qa-assets`](https://github.com/opensy-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/opensy-core/qa-assets
...
an instrumentalized libc++, then using it to build [OpenSY dependencies
from source](../depends/README.md) and finally the OpenSY fuzz binary
...
the qa-assets repo
(https://github.com/opensy-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`opensy-core/qa-assets`](https://github.com/opensy-core/qa-assets) repo.

Every single pull request submitted against the OpenSY repo is automatically tested against all inputs in the [`opensy-core/qa-assets`](https://github.com/opensy-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make OpenSY more robust.
...
# Fuzzing OpenSY using Honggfuzz
...
OpenSY participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/opensy-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=opensy-core%20status:open).

OpenSY follows its [security disclosure policy](https://opensy.net/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_opensy-core/latest).
