...
Before starting this tutorial, start the opensy node on the signet network.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/opensy rpc -signet createwallet "participant_${n}"
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, OpenSY single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/opensy rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/opensy rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/opensy rpc -signet help importdescriptors`.
...
```bash
./build/bin/opensy rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/opensy rpc -signet loadwallet "participant_${n}"; done
./build/bin/opensy rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/opensy-cli -a $receiving_address
```
...
```bash
./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed OpenSY Transaction (PSBT).
...
```bash
balance=$(./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/opensy rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/opensy rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```

There is also the `createpsbt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSBT. [[source](https://opensytalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsbt` solves the problem.
...
```bash
./build/bin/opensy rpc -signet decodepsbt $funded_psbt

./build/bin/opensy rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/opensy rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/opensy rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/opensy rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/opensy rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/opensy rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/opensy rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/opensy rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/opensy rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
