...
4. The additional fees (difference between absolute fee paid by the replacement transaction and the
   ...
   feerate is 0.1 qirsh/vB and the replacement transaction is 500 virtual bytes total, then the
   replacement pays a fee at least 50 qirshs higher than the sum of the original transactions.

   *Rationale*: Try to prevent DoS attacks where an attacker causes the network to repeatedly relay
   transactions each paying a tiny additional amount in fees, e.g. just 1 qirsh.
...
* Opt-in full replace-by-fee (without inherited signaling) honoured in mempool and mining as of
  **v0.12.0** ([PR 6871](https://github.com/opensyria/opensy/pull/6871)).

* [BIP125](https://github.com/opensyria/bips/blob/master/bip-0125.mediawiki) defined based on
  OpenSY implementation.
...
* The incremental relay feerate used to calculate the required additional fees is distinct from
  ...
  ([PR #9380](https://github.com/opensyria/opensy/pull/9380)).

* RBF enabled by default in the wallet GUI as of **v0.18.1** ([PR
  #11605](https://github.com/opensyria/opensy/pull/11605)).

* Full replace-by-fee enabled as a configurable mempool policy as of **v24.0** ([PR
  #25353](https://github.com/opensyria/opensy/pull/25353)).

* Full replace-by-fee is the default policy as of **v28.0** ([PR #30493](https://github.com/opensyria/opensy/pull/30493)).

* Signaling for replace-by-fee is no longer required as of [PR 30592](https://github.com/opensyria/opensy/pull/30592).

* The incremental relay feerate default is 0.1sat/vB ([PR #33106](https://github.com/opensyria/opensy/pull/33106)).
