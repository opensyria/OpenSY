...
* Update opensy.conf and commit changes if they exist, see [gen-opensy-conf.sh](/contrib/devtools/README.md#gen-opensy-confsh).
...
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `m_assumeutxo_data` array should be appended to with the values returned by calling `opensy-cli -rpcclienttimeout=0 -named dumptxoutset utxo.dat rollback=<height or hash>`
...
- Translations on Transifex:
    ...
    - Create [a new resource](https://app.transifex.com/opensy/opensy/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/opensy_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://app.transifex.com/opensy/communication/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/opensy/opensy/master/src/qt/locale/opensy_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)
...
- Prune inputs from the qa-assets repo (See [pruning
  inputs](https://github.com/opensy-core/qa-assets#pruning-inputs)).
...
To tag the version (or release candidate) in git, use the `make-tag.py` script from [opensy-maintainer-tools](https://github.com/opensy-core/opensy-maintainer-tools). From the root of the repository run:

    ../opensy-maintainer-tools/make-tag.py v(new version, e.g. 25.0)
...
Open a draft of the release notes for collaborative editing at https://github.com/opensy-core/opensy-devwiki/wiki.
...
Checkout the OpenSY version you'd like to build:
...
Then open a Pull Request to the [guix.sigs repository](https://github.com/opensy-core/guix.sigs).
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf opensy-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf opensy-${VERSION}-win64-codesigning.tar.gz
...
However if this is done, once the release has been tagged in the opensy-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./opensy-detached-sigs
...
- Detached signatures will then be committed to the [opensy-detached-sigs](https://github.com/opensy-core/opensy-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- After uploading release candidate binaries, notify the opensy-core-dev mailing list and
  opensy-dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Update repositories
  ...
  - Update packaging repo
      ...
      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.opensycore.opensy-qt/pull/2
