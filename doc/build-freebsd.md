...
This guide describes how to build opensyd, command-line utilities, and GUI on FreeBSD.
...
To build OpenSY without the wallet, use `-DENABLE_WALLET=OFF`.
...
### 2. Clone OpenSY Repo
Now that `git` and all the required dependencies are installed, let's clone the OpenSY repository to a directory. All build scripts and commands will run from this directory.
```bash
...
OpenSY can provide notifications via ZeroMQ. If the package is installed, support will be compiled in.
```bash
pkg install libzmq4
```
