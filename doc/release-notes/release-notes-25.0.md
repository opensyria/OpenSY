...
```sh
opensy-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
opensy-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, OpenSY will now return an error at startup. (#25574)
...
before OpenSY has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching opensyd with the configuration option
