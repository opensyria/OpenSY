OpenSY version 0.18.0 is now available from:

  <https://opensy.net/bin/opensy-core-0.18.0/>
...
`/Applications/OpenSY-Qt` (on Mac) or `opensyd`/`opensy-qt` (on
Linux).
...
recommended to use OpenSY on unsupported systems.
...
10.10. Additionally, OpenSY does not yet change appearance when
...
`contrib/init/opensyd.service`, it has been changed to use
`/var/lib/opensyd` as the data directory instead of
`~opensy/.opensy`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/opensyd` will
exist has enough space (check using `df -h /var/lib/opensyd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/opensyria/opensy/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://opensy.net/en/contact/) the OpenSY
...
The systemd init file (`contrib/init/opensyd.service`) has been changed
to use `/var/lib/opensyd` as the data directory instead of
`~opensy/.opensy`. This change makes OpenSY more consistent with
...
`datadir` option in `/etc/opensy/opensy.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/opensy/opensy.conf`.


Documentation
-------------

- A new short [document](https://github.com/opensyria/opensy/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/opensyria/opensy/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/opensyria/opensy/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/opensyria/opensy/blob/master/doc/opensy-conf.md)
  about the `opensy.conf` file describes how to use it to configure
  OpenSY.

- A new document introduces OpenSY's BIP174 [Partially-Signed
  OpenSY Transactions
  (PSBT)](https://github.com/opensyria/opensy/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/opensyria/opensy/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent OpenSY-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  OpenSY to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/opensyria/opensy/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/opensyria/opensy/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when OpenSY knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of OpenSY, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/opensyria/opensy/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/opensyria/opensy/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart opensyd with the `-deprecatedrpc=generate`
...
- A new `opensy-wallet` tool is now distributed alongside OpenSY
...
This section describes planned changes to OpenSY that may affect
other OpenSY software and services.

- Since version 0.16.0, OpenSY’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with OpenSY 0.20 (expected about
  a year after 0.18), OpenSY will default to native segwit
  ...
  to bech32 addresses, and if the OpenSY project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in OpenSY 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any OpenSY
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, OpenSY
...
- A sub-project of OpenSY now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with OpenSY.  See their [project
  page](https://github.com/opensy-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to OpenSY's own implementation, although entropy
  gathered by OpenSY is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves OpenSY a
...
- On macOS, OpenSY now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `opensy_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libopensy_server.a` (jnewbery)
...
- #13926 [Tools] opensy-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like opensyd (MarcoFalke)
...
- #14521 qt, docs: Fix `opensy-qt -version` output formatting (hebasto)
...
- #11625 Add OpenSYApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libopensy_server` (laanwj)
...
- #15154 Configure: opensy-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/opensy-wallet (Sjors)
...
- #11526 Visual Studio build configuration for OpenSY (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libopensy_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in opensy-util-test.py (isghe)
...
- #12775 Integration of property based testing into OpenSY (Christewart)
...
- #12246 Bugfix: Only run opensy-tx tests when opensy-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_opensy` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_opensy` (MarcoFalke)
...
- #14186 opensy-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_opensy.conf` message on startup if conf file exists (leishman)
...
- #12255 Update opensy.service to conform to init.md (dongcarl)
...
- #14497 Add doc/opensy-conf.md (hebasto)
...
- #15354 Add missing `opensy-wallet` tool manpages (MarcoFalke)
...
- #15765 correct opensyconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support opensy folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/opensy/).
