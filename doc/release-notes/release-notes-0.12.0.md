...
installer (on Windows) or just copy over /Applications/OpenSY-Qt (on Mac) or
opensyd/opensy-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of OpenSY or other software:
...
pre-0.12 versions of OpenSY or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start OpenSY version 0.11 or
earlier.
...
ECDSA signatures inside OpenSY transactions now use validation using
[libsecp256k1](https://github.com/opensy-core/secp256k1) instead of OpenSSL.
...
(https://github.com/opensyria/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of OpenSY had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of OpenSY was to raise the default minimum
relay fee.

OpenSY 0.12 will have a strict maximum size on the mempool. The
...
OpenSY 0.12 also introduces new default policy limits on the length and
...
OpenSY 0.12 nodes. OpenSY will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/opensyria/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in OpenSY 0.12 does not yet have support for
...
This allows running opensyd without having to do any manual configuration.
...
OpenSY has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. OpenSY will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In OpenSY 0.12, when mempool limit has been reached a higher minimum
...
entirely in OpenSY 0.13, and it is at this time undecided whether the
...
OpenSY has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
OpenSY automatically creates a hidden service to listen on, without
manual configuration. OpenSY will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if OpenSY is listening, and
...
OpenSYd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals OpenSY
Core to use floating fees. By default, OpenSY will use floating
fees.
...
(default: `0.0002` SYL/kB).

At all times, OpenSY will cap fees at `-maxtxfee=<x>` (default:
0.10) SYL.
Furthermore, OpenSY will never create transactions paying less than
the current minimum relay fee.
Finally, a user can set the minimum fee rate for all transactions with
`-mintxfee=<i>`, which defaults to 1000 qirshs per kB.
...
`opensy.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
111](https://github.com/opensyria/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/opensyria/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /opensyrpc>
        ...
        # optional bypass opensyd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< opensyrpc:<password>
    </Location>
...
Restarting opensyd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of OpenSY-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new opensy-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include opensy-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix opensy-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of opensy-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override OPENSYD and OPENSYCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [opensy-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add OPENSYD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libopensyconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "SYL" (Ross Nicoll)
