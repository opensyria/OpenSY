...
- [Miniscript](https://opensy.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://opensy.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `opensyd` and `opensy-qt` will now raise an error on startup
 if a datadir that is being used contains a opensy.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a opensy.conf file. The error message is just a diagnostic intended
 ...
 the opensy.conf contained in it. (#27302)
...
- A new `opensyconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `opensyconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of OpenSY and import the data into a new wallet.
...
```sh
src/opensy-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/opensy-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://opensy.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). OpenSY would
...
- Bash completion files have been renamed from `opensy*.bash-completion` to
  `opensy*.bash`. This means completions can be automatically loaded on demand
