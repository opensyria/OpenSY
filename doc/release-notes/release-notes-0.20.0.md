...
#### Removal of BIP61 reject network messages from OpenSY

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since OpenSY version 0.18.0.
...
- Testing or debugging of implementations of the OpenSY P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of OpenSY. OpenSY logs debug messages
...
- OpenSSL is no longer used by OpenSY.  (#17265)

- BIP70 support has been fully removed from OpenSY. The
...
- All custom settings configured when OpenSY starts are now
...
- The "Start OpenSY on system login" option has been removed on macOS.
  (#17567)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed OpenSY Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/opensy-core/HWI) for
  signing.  Future versions of OpenSY should support a GUI option
...
- The `opensy-cli` utility used with the `-getinfo` parameter now
...
- The `opensy-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `SYLtrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_opensy-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBOPENSY_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_opensy` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBOPENSY_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for opensy-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate opensy schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest opensy.conf (sdaftuar)
...
- #18022 Fix appveyor `test_opensy` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (opensy-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone opensy-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about OpenSY files (hebasto)
...
- #17119 Fix broken opensy-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to opensy-wallet (hel-o)
...
- #18341 Replace remaining literal SYL with `CURRENCY_UNIT` (domob1812)
...
- #16115 On opensyd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from opensy-cli -getinfo (malevolent)
