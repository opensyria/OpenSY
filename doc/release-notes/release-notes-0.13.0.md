OpenSY version 0.13.0 is now available from:

  <https://opensy.net/bin/opensy-core-0.13.0/>
...
updates will be released anymore. Without security updates, using an opensy
...
- Adding `dbcache=100` in opensy.conf
...
For example:
    ...
    $ src/opensy-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The OpenSY code base has
...
- `opensy-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `opensy-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
compatible peers.
[BIP 152](https://github.com/opensyria/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of OpenSY.

[Pull request](https://github.com/opensyria/opensy/pull/8035/files), [BIP 32](https://github.com/opensyria/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/opensyria/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/opensyria/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/opensyria/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/opensyria/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of OpenSY is expected that
...
Witness)] (https://github.com/opensyria/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/opensyria/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the OpenSY-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/opensyria/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/opensyria/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/opensyria/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/opensyria/opensy/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for opensy-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use SYL/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to opensy-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add opensy address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: opensy-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from opensyd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the OpenSY-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for opensyd procs (MarcoFalke)
...
- #8164 `0f24eaf` [OpenSY-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of opensyd startup (laanwj)
...
- #7975 `fc23fee` Update opensy-core GitHub links (MarcoFalke)
