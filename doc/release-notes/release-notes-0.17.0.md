...
OpenSY is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `opensy.conf` file, not inside included files or from
  ...
  as opensy.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the opensy.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=opensy
    test.uacomment=opensy-testnet
...
be used in V0.17 by starting opensyd with the '-deprecatedrpc=accounts'
...
[CoinJoin](https://opensytalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed OpenSY transaction goes through the
following steps:
...
- An **Extractor** produces a valid OpenSY transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in SYL. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When opensy is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If opensy is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart opensyd with
...
- When running opensyd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If opensyd
...
- #11962 `0a01843` add seed.opensy.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in SYL (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI opensy:// (krab)
...
- #13264 `2a7c53b` Qirsh unit (GreatSock)
...
- #13528 `b877c39` Move OpenSYGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libopensy-util to libopensy-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for opensy-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling opensy-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use OpenSYTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to opensy.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use OpenSYTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use OpenSYTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use OpenSYTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for OpenSYTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_opensy` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.opensy is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_opensy` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in opensyd systemd service file (Flowdalic)
- #12652 `55f490a` opensy-cli: Provide a better error message when opensyd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to opensyd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for opensy address in translations (kaplanmaxe)
...
- #13603 `453ae5e` opensy-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix opensy-cli --version (Empact)
...
- #12877 `2b54155` Use opensyd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update opensy-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed opensy-cli -help output for help2man (hebasto)
