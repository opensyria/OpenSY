...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. OpenSY will no longer
...
- A new `opensy-cli -netinfo` command provides a network peer connections
...
- A new `opensy-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `opensy-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `opensy-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when OpenSY has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `opensy-wallet` tool which performs the salvage
...
OpenSY will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`opensy.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
OpenSY will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `opensy-cli createwallet` or `opensy-wallet
...
Users who already have a OpenSY wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of OpenSY.
...
The change to using descriptors changes the default derivation paths used by OpenSY
...
- A new `fee_rate` parameter/option denominated in qirshs per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in SYL per 1,000
  vbytes (SYL/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from SYL/kB
  to sat/vB and the help documentation in SYL/kB is updated to SYL/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from SYL/kvB to sat/vB and the
  ...
  difference between SYL/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in SYL/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time OpenSY is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `opensy.conf`
...
- #19284 Add seed.opensy.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in opensy-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add opensy-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in opensy-cli (laanwj)
...
### Build system
- #18504 Drop opensy-tx and opensy-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in OPENSY_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for opensy-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz OpenSY using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreopensy)
...
- #18817 Document differences in opensyd and opensy-qt locale handling (practicalswift)
