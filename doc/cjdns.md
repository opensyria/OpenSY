# CJDNS support in OpenSY

It is possible to run OpenSY over CJDNS, an encrypted IPv6 network that
...
done outside of OpenSY, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the OpenSY network and individual nodes.
...
before it will work with your OpenSY node. This is described in steps
...
## Run OpenSY with CJDNS

Once you are connected to the CJDNS network, the following OpenSY
...
When enabled, this option tells OpenSY that it is running in an
...
IPv6 local network. This helps OpenSY perform better address management:
...
CJDNS support was added to OpenSY in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `opensy-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/opensyria/opensy/blob/master/doc/tor.md) for
details.

## CJDNS-related information in OpenSY

There are several ways to see your CJDNS address in OpenSY:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `opensy-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `opensy-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `opensy-cli help getnodeaddresses` for details.
