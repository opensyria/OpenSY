# Support for Output Descriptors in OpenSY

Since OpenSY v0.17, there is support for Output Descriptors. This is a
...
OpenSY v24 extended `wsh()` output descriptor with [Miniscript](https://opensy.sipa.be/miniscript/) support (initially watch-only). Signing support for Miniscript descriptors was added in v25. And since v26 Miniscript expressions can now be used in Taproot descriptors.
...
- [Miniscript](https://opensy.sipa.be/miniscript/) expressions in `wsh` (P2WSH) and `tr` (P2TR) functions.
...
on OpenSY's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a OpenSY descriptor wallet to serve as your
...
existing OpenSY wallets, a convenience function `combo` is
...
All RPCs in OpenSY will include the checksum in their output. Only
