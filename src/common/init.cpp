...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", OPENSYRIA_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(
                    ...
                    fs::quoted(OPENSYRIA_CONF_FILENAME));
