                     ...
                     TxVerbosity verbosity = TxVerbosity::SHOW_DETAILS)
{
    ...
    // Call into TxToUniv() in opensyria-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in opensyria-common, so we query them here and push the
        ...
        {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs specified as key-value pairs.\n"
                ...
                "                             accepted as second parameter.",
            {
                {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                    {
                        {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the opensyria address, the value (float or string) is the amount in " + CURRENCY_UNIT},
                    },
                },
                {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                    {
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed OpenSyria transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{
        "combinepsbt",
        "Combine multiple partially signed OpenSyria transactions into one transaction.\n"
