...
{
    GUIUtil::setClipboard(OpenSyriaUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    OpenSyriaUnit nDisplayUnit = OpenSyriaUnit::BTC;
    ...
    l2->setText(OpenSyriaUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(OpenSyriaUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(OpenSyriaUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(OpenSyriaUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    OpenSyriaUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, OpenSyriaUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, OpenSyriaUnits::format(nDisplayUnit, nSum));
