...
public:
    ...
    QString describe(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, OpenSyriaUnit unit)
    {
        ...
        platformStyle(_platformStyle)
{
    ...
    columns << QString() << tr("Date") << tr("Type") << tr("Label") << OpenSyriaUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    columns[Amount] = OpenSyriaUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, OpenSyriaUnits::SeparatorStyle separators) const
{
    QString str = OpenSyriaUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false, separators);
    if(showUnconfirmed)
    {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, OpenSyriaUnits::SeparatorStyle::ALWAYS);
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, OpenSyriaUnits::SeparatorStyle::NEVER));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, OpenSyriaUnits::SeparatorStyle::NEVER);
    case StatusRole:
